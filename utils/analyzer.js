...
async function findPortsInDir(baseDir, targetDir, portNamesPattern, defaultPort) {
  const regexList = [
    new RegExp(`^(?!\\s*(?:#|\\/\\/)).*(?:${portNamesPattern})\\s*[:=]\\s*["']?(\\d+)["']?`, 'gim'),
    new RegExp(`^(?!\\s*(?:#|\\/\\/)).*(?:process\\.env\\.)?(?:${portNamesPattern})\\s*\\|\\|\\s*(\\d+)`, 'gim'),
    new RegExp(`^(?!\\s*(?:#|\\/\\/)).*port\\s*[:=]\\s*["']?(\\d+)["']?`, 'gim'),
    new RegExp(`^(?!\\s*(?:#|\\/\\/)).*--inspect(?:-brk)?=(?:[^:]+:)?(\\d+)`, 'gim'),
    new RegExp(`(?:^|\\s)(?:--port|-p)\\s*[=:]?\\s*(\\d+)`, 'gim'),
    new RegExp(`\\bport\\b.{0,15}?(?<![a-zA-Z0-9.-])(\\d{2,5})\\b`, 'gim'),
